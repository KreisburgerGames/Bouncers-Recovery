...
public class Process : IDisposable
{
	...
	[DllImport("KS_Diagnostics_Process", EntryPoint ="TotalProcessorTime")]
	static extern long GetTotalProcessorTime(IntPtr ptr);
	public TimeSpan TotalProcessorTime => TimeSpan.FromTicks(GetTotalProcessorTime(ptr));
	...
	{
		this.ptr = ptr;
		this.StandardInput = new FakeStandardInput(ptr);
	}
...
public class FakeStandardInput
{
	...
	[DllImport("KS_Diagnostics_Process")]
	static extern void StandardInput_Close(IntPtr ptr);
	public void Close()
	{
		StandardInput_Close(procPtr);
	}
	public FakeStandardInput(IntPtr ptr)
	{
		this.procPtr = ptr;
		BaseStream = new FakeBaseStream(ptr);
	}
	public readonly FakeBaseStream BaseStream;
	public class FakeBaseStream
	{
		readonly IntPtr procPtr;
		public FakeBaseStream(IntPtr ptr)
		{
			procPtr = ptr;
		}
		[DllImport("KS_Diagnostics_Process")]
		static extern void StandardInput_BaseStream_Write(IntPtr ptr, IntPtr ptrArrBytes, int offset, int count);
		
		public  void Write(byte[] arr, int offset, int count)
		{
			GCHandle gch = GCHandle.Alloc(arr, GCHandleType.Pinned);
			IntPtr ptrArr = Marshal.UnsafeAddrOfPinnedArrayElement(arr, 0);
			StandardInput_BaseStream_Write(procPtr, ptrArr, offset, count);
			gch.Free();
		}
		
		/// <summary>
		/// Memory allocation free version
		/// </summary>
		...
		{
			StandardInput_BaseStream_Write(procPtr, arayrPointer, offset, count);
		}
		
		[DllImport("KS_Diagnostics_Process")]
		static extern void StandardInput_BaseStream_Flush(IntPtr ptr);
		public void Flush()
		{
			StandardInput_BaseStream_Flush(procPtr);
		}
		
		[DllImport("KS_Diagnostics_Process")]
		static extern void StandardInput_BaseStream_Close(IntPtr ptr);
		public void Close()
		{
			StandardInput_BaseStream_Close(procPtr);
		}
	}
}
}